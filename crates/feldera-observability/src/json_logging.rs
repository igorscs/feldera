...
pub enum LogIdentity {
    ...
    Service { service_name: Option<String> },
    /// Pipeline log source.
    Pipeline {
    ...
    N: for<'a> FormatFields<'a> + 'static,
{
    fn format_event(
        ...
        let mut feldera_service = match &self.identity {
            LogIdentity::Service { service_name } => service_name.clone(),
        ...
        if let Some(value) = visitor
            ...
            .or_else(|| visitor.fields.remove("pipeline"))
        {
        ...
        if let Some(value) = visitor
            .fields
            .remove("pipeline-id")
            .or_else(|| visitor.fields.remove("pipeline_id"))
        {
...
pub fn init_service_logging(
    ...
    feldera_service: impl Into<String>,
    env_filter: EnvFilter,
) -> Result<(), tracing_subscriber::util::TryInitError> {
    init_logging(
        service_name,
        LogIdentity::Service {
            service_name: Some(sanitize_pipeline_name(feldera_service.into())),
        },
        env_filter,
    )
}
